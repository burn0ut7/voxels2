...
$fullRequiredScenarios = @(
	...
	'gpu_far_aimed_brush_edits_20hz',
...
$requiredMetrics = @(
	...
	'collision_snapshot_wait_ms', 'collision_snapshot_copy_ms', 'collision_worker_mesh_ms', 'collision_model_build_ms', 'collision_publication_ms',
...
if ( $report.suite_version -ne 39 ) { Add-Failure "Expected suite version 39, found '$($report.suite_version)'" }
...
foreach ( $scenario in $scenarios )
{
	...
	elseif ( $scenario.scenario -eq 'high_speed_collision_streaming' )
	{
		if ( [long]$scenario.calls_player_traversal_updates -le 0 ) { Add-Failure "$context did not move the actual player" }
		if ( [long]$scenario.calls_collision_builds_completed -le 0 ) { Add-Failure "$context recorded no completed predictive collision builds" }
	}
	...
	elseif ( $scenario.scenario -in @('gpu_realtime_surface_edits_20hz', 'gpu_accumulated_surface_edits_20hz') )
	{
		if ( $scenario.gpu_terrain_available -ne $true ) { Add-Failure "$context has no persistent GPU terrain diagnostics" }
		...
		if ( $scenario.scenario -eq 'gpu_accumulated_surface_edits_20hz' -and [int]$scenario.gpu_edit_queue_count -lt 160 ) { Add-Failure "$context did not retain the accumulated edit history" }
	...
	elseif ( $scenario.scenario -eq 'gpu_far_aimed_brush_edits_20hz' )
	{
		if ( $scenario.gpu_terrain_available -ne $true ) { Add-Failure "$context has no persistent GPU terrain diagnostics" }
		if ( [int]$scenario.edits -ne 80 -or [int]$scenario.changed_chunk_events -le 0 ) { Add-Failure "$context did not execute eighty far aimed brush digs" }
		if ( [long]$scenario.calls_brush_raycasts -ne 80 -or [long]$scenario.calls_brush_raycast_samples -le 0 ) { Add-Failure "$context did not raycast every far aimed brush dig" }
		if ( [long]$scenario.calls_brush_raycast_edit_candidates -ge 19200 ) { Add-Failure "$context replayed the full accumulated journal for far brush raycasts" }
		if ( [long]$scenario.calls_collision_builds_started -ne 0 ) { Add-Failure "$context built player collision for far-away brush edits" }
		if ( [int]$scenario.gpu_edit_queue_count -lt 320 ) { Add-Failure "$context did not retain all far aimed brush edits in the GPU journal" }
		if ( [int]$scenario.gpu_terrain_clipbox_dropped_work -ne 0 -or [int]$scenario.gpu_terrain_clipbox_transition_dependency_mismatches -ne 0 ) { Add-Failure "$context dropped or mismatched far aimed brush work" }
		if ( [int]$scenario.gpu_terrain_pending_request_count -ne 0 -or [int]$scenario.gpu_terrain_pending_publication_count -ne 0 ) { Add-Failure "$context completed with pending far brush work" }
	}
	elseif ( $scenario.scenario -eq 'gpu_cpu_collision_rebuild_cost' )
	{
		if ( [long]$scenario.calls_collision_builds_started -le 0 ) { Add-Failure "$context started no standalone CPU collision builds" }
		if ( [long]$scenario.calls_collision_uploads -ne [long]$scenario.calls_collision_builds_started ) { Add-Failure "$context did not publish every completed CPU collision build" }
		if ( [long]$scenario.collision_triangles -le 0 ) { Add-Failure "$context published no collision triangles" }
		if ( [double]$scenario.collision_snapshot_copy_ms -le 0 -or [double]$scenario.collision_worker_mesh_ms -le 0 -or [double]$scenario.collision_model_build_ms -le 0 -or [double]$scenario.collision_publication_ms -le 0 ) { Add-Failure "$context is missing collision phase timings" }
	}
	...
	elseif ( $scenario.scenario -notlike 'player_*_streaming' -and $scenario.scenario -ne 'high_speed_collision_streaming' -and $scenario.scenario -notlike 'phase5_*' -and [long]$scenario.calls_safety_players_repositioned -le 0 )
	{
