public sealed class VoxelTerrainBenchmark : Component
{
	...
	private string BuildDashboardHtml()
	{
		...
		var comparisonOptions = string.Join( string.Empty, ComparisonMetrics.Select( metric => $"<option value=change_{metric.Name}_pct>{ComparisonMetricLabel( metric.Name )} change (%)</option>" ) );
		return "<!doctype html><html><head><meta charset=\"utf-8\"><title>Voxel Terrain Benchmark</title><style>" +
			...
			"const colors=['#82d8a7','#72a7ff','#ffc46b','#df86ff','#ff7f86','#73d9dc'];function draw(){const select=document.getElementById('metric'),key=select.value,c=document.getElementById('chart'),x=c.getContext('2d'),isSignedChange=key.startsWith('change_')&&key!=='change_max_abs_pct';document.getElementById('chartContext').textContent=isSignedChange?'Percentage change from the previous compatible run; this is not raw FPS or milliseconds.':'Raw metric values; the vertical scale cannot go below zero.';x.clearRect(0,0,c.width,c.height);const available=rows.filter(r=>r[key]!==undefined&&Number.isFinite(Number(r[key]))),pad=55,w=c.width-pad*2,h=c.height-pad*2,values=available.map(r=>Number(r[key])),min=isSignedChange?Math.min(0,...values):0,max=Math.max(0,...values),range=Math.max(1,max-min),y=v=>pad+(max-v)/range*h;x.strokeStyle='#40516a';x.beginPath();x.moveTo(pad,pad);x.lineTo(pad,pad+h);x.stroke();x.strokeStyle='#647792';x.beginPath();x.moveTo(pad,y(0));x.lineTo(pad+w,y(0));x.stroke();const scenarios=[...new Set(available.map(r=>r.scenario))];scenarios.forEach((s,si)=>{const data=available.filter(r=>r.scenario===s);x.strokeStyle=colors[si%colors.length];x.fillStyle=x.strokeStyle;x.beginPath();data.forEach((r,i)=>{const px=pad+(runs.indexOf(r.run_id)/Math.max(1,runs.length-1))*w,py=y(Number(r[key]));i?x.lineTo(px,py):x.moveTo(px,py);x.fillRect(px-3,py-3,6,6)});x.stroke();x.fillText(s,pad+si*190,20)});x.fillStyle='#9fb0c5';x.fillText(max.toFixed(2),5,pad+5);x.fillText(min.toFixed(2),5,pad+h+5);runs.forEach((r,i)=>{if(i%Math.max(1,Math.ceil(runs.length/8))===0)x.fillText(r,pad+i/Math.max(1,runs.length-1)*w-30,c.height-10)})}document.getElementById('metric').onchange=draw;draw();" +
			"const cols=['run_id','suite_version','suite_complete','revision','scenario','change_max_abs_pct','outlier_detected','outlier_metrics','reproduction_status','visual_coherence_violation_frames','avg_fps','one_percent_low_fps','frame_p95_ms','frame_max_ms','stutter_events','edit_call_p95_ms','post_edit_settle_ms','allocated_bytes','network_avg_ms','authoritative_sdf_bytes','visual_triangles','worker_mesh_ms'];document.getElementById('table').innerHTML='<tr>'+cols.map(c=>`<th>${c}</th>`).join('')+'</tr>'+rows.slice().reverse().map(r=>'<tr>'+cols.map(c=>`<td>${r[c]??''}</td>`).join('')+'</tr>').join('');</script></body></html>";
	}
	...
	private static string ComparisonMetricLabel( string name ) => name switch
	{
		"avg_fps" => "Average FPS",
		"one_percent_low_fps" => "1% low FPS",
		"frame_p95_ms" => "Frame p95",
		"frame_max_ms" => "Worst frame",
		"gpu_p95_ms" => "GPU p95",
		"edit_call_p95_ms" => "Brush-call p95",
		"post_edit_settle_ms" => "Post-edit settle",
		"allocated_bytes" => "Managed allocations",
		"visual_batch_ms" => "Visual batch",
		"worker_mesh_ms" => "Worker mesh",
		"upload_ms" => "Main upload",
		_ => name
	};
	...
	private sealed class FrameSampler
	{
		...
		public ScenarioResult Complete( VoxelTerrainDiagnostics diagnostics, VoxelCallCountSnapshot callCounts, double elapsedMilliseconds )
		{
			...
			return new ScenarioResult
			{
				Name = Name,
				Description = Description,
				Passed = diagnostics.FailedVisualChunks == 0 && diagnostics.PendingVisualBuilds == 0 && diagnostics.PendingCollisionBuilds == 0 && !diagnostics.PlayerSafetyActive && _exceptions == 0 && _visualCoherenceViolationFrames == 0,
				EditCount = EditCount,
