...
internal static class VoxelGpuPhase2BProof
{
	...
	public static VoxelGpuPhase2BProofResult ValidateStatic( string test, VoxelGpuTerrainDiagnostics diagnostics, int expectedBlocks, int minimumReadbacks = 1 )
	{
		...
		if ( string.IsNullOrEmpty( failure ) && diagnostics.ResidentBlocks != expectedBlocks ) failure = $"published {diagnostics.ResidentBlocks} residents, expected {expectedBlocks}";
		...
		if ( string.IsNullOrEmpty( failure ) && diagnostics.RequestToVisible.Count != expectedBlocks ) failure = $"recorded {diagnostics.RequestToVisible.Count} request-to-visible samples, expected {expectedBlocks}";
		...
		if ( string.IsNullOrEmpty( failure ) && diagnostics.AllocationFailures != 0 ) failure = $"mesh pool reported {diagnostics.AllocationFailures} allocation failures";
