...
internal static class VoxelGpuPhase3BProof
{
	public static VoxelGpuPhase3BProofResult ValidateMovement( string test, VoxelGpuTerrainDiagnostics gpu, VoxelTerrainDiagnostics terrain )
	{
		...
		if ( string.IsNullOrEmpty( failure ) && gpu.ResidentBlocks != gpu.DesiredBlocks ) failure = $"published {gpu.ResidentBlocks} residents for {gpu.DesiredBlocks} desired blocks";
		...
		if ( string.IsNullOrEmpty( failure ) && gpu.AllocationFailures != 0 ) failure = $"encountered {gpu.AllocationFailures} GPU allocation failures";
