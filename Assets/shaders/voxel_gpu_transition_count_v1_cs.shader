...
// Transvoxel shared fine-face density with stable near-zero classification and sparse-edit ownership.
CS
{
	#include "system.fxc"
	...
	[numthreads(64,1,1)] void MainCs( uint3 id : SV_DispatchThreadID )
	{
		...
		[loop]for(uint cell=0;cell<(uint)TransitionCellCount;cell++)
		{
			...
			[unroll]for(uint sample=0;sample<9;sample++)Samples[baseIndex+sample]=EvaluateIndexedTerrainDensity(FineSample(request,sample,cell),(uint)request.FineOrigin.w,(uint)request.CoarseOrigin.w,SdfClampDistance,SimplexFrequency,SimplexAmplitude,SimplexBaseHeight,SimplexSeed);
