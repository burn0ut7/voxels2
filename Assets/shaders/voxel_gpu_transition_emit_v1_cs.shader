...
CS
{
	...
	float3 Normal(TransitionRequest request,float3 p,uint sampleA,uint sampleB,uint cellIndex,float t){float e=(float)request.FineStep;float3 a=WorldSample(request,sampleA,cellIndex),b=WorldSample(request,sampleB,cellIndex),pa=lerp(a,b,t);uint editIndexOffset=(uint)request.FineOrigin.w,editIndexCount=(uint)request.CoarseOrigin.w;float3 g=float3(Density(pa+float3(e,0,0),editIndexOffset,editIndexCount)-Density(pa-float3(e,0,0),editIndexOffset,editIndexCount),Density(pa+float3(0,e,0),editIndexOffset,editIndexCount)-Density(pa-float3(0,e,0),editIndexOffset,editIndexCount),Density(pa+float3(0,0,e),editIndexOffset,editIndexCount)-Density(pa-float3(0,0,e),editIndexOffset,editIndexCount));float d=dot(g,g);if(d<1e-12)return FaceNormal(request.Face);return g*rsqrt(d);}
	[numthreads(64,1,1)] void MainCs(uint3 id:SV_DispatchThreadID)
	{
		...
		[loop]for(uint cell=0;cell<(uint)TransitionCellCount;cell++)
		{
			...
			uint triBase=TriangleOffset+cls*36;bool tableFlip=(cellClass&0x80)!=0;[unroll]for(uint i=0;i<36;i+=3){if(i>=indexCount)break;uint a=Lookup[triBase+i],b=Lookup[triBase+i+1],c=Lookup[triBase+i+2],va=a+vertexCursor,vb=b+vertexCursor,vc=c+vertexCursor;VoxelGpuVertex pa=OutputVertices[allocation.VertexOffset+va],pb=OutputVertices[allocation.VertexOffset+vb],pc=OutputVertices[allocation.VertexOffset+vc];float3 fn=cross(pb.Position-pa.Position,pc.Position-pa.Position),vn=pa.Normal+pb.Normal+pc.Normal;bool forward=dot(vn,vn)>1e-8?dot(fn,vn)>=0:tableFlip;uint target=allocation.IndexOffset+indexCursor+i;if(forward){OutputIndices[target]=va;OutputIndices[target+1]=vb;OutputIndices[target+2]=vc;}else{OutputIndices[target]=vc;OutputIndices[target+1]=vb;OutputIndices[target+2]=va;}}
