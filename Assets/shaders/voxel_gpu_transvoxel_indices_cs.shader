...
CS
{
	...
	StructuredBuffer<uint> EdgeGroupSums < Attribute( "EdgeGroupSums" ); >; StructuredBuffer<uint> CellGroupSums < Attribute( "CellGroupSums" ); >; StructuredBuffer<uint> BlockCounts < Attribute( "BlockCounts" ); >;
	RWStructuredBuffer<uint> OutputIndices < Attribute( "OutputIndices" ); >;
	int ChunkSize < Attribute( "ChunkSize" ); >; int SampleSize < Attribute( "SampleSize" ); >; int CellCount < Attribute( "CellCount" ); >; int EdgeSlotCount < Attribute( "EdgeSlotCount" ); >; int EdgeGroupCount < Attribute( "EdgeGroupCount" ); >; int CellGroupCount < Attribute( "CellGroupCount" ); >; int DescriptorOffset < Attribute( "DescriptorOffset" ); >; int BatchSize < Attribute( "BatchSize" ); >;
	...
	{
		if(id.x>=(uint)CellCount*(uint)BatchSize)return;uint block=id.x/(uint)CellCount,local=id.x-block*(uint)CellCount,code=Cells[id.x].x;if(code==0||code==255)return;uint3 cell=Decode3D(local,ChunkSize);uint cls=RegularLookup[code],counts=RegularLookup[RegularGeometryCountsOffset+cls],vc=counts>>4,tc=counts&0xf,verts[12];
		for(uint v=0;v<vc;v++){uint edge=EdgeSlot(cell,RegularLookup[RegularVertexDataOffset+code*12+v]);verts[v]=BlockCounts[(uint)DescriptorOffset+block*4]+EdgeGroupSums[block*(uint)EdgeGroupCount+edge/256]+EdgeVertexIds[block*(uint)EdgeSlotCount+edge];}uint output=BlockCounts[(uint)DescriptorOffset+block*4+1]+CellGroupSums[block*(uint)CellGroupCount+local/256]+Cells[id.x].z,triBase=RegularTriangleIndicesOffset+cls*15;
