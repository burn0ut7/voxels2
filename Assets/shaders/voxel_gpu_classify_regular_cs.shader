...
// Transvoxel secondary-position ownership; regular boundary cells remain classified.
CS
{
	#include "system.fxc"
	// Shared regular-cell classification for production and conformance.
	// Boundary cells stay authoritative; their emitted vertices are deformed to
	// make room for the Transvoxel companion.
	...
	[numthreads(64,1,1)] void MainCs(uint3 id:SV_DispatchThreadID)
	{
		...
		if(id.x>=(uint)CellCount*(uint)BatchSize)return;uint block=id.x/(uint)CellCount,local=id.x-block*(uint)CellCount;uint3 cell=Decode3D(local,ChunkSize);uint code=0;[unroll]for(uint c=0;c<8;c++)if(Distance(block,int3(cell+Corners[c]))<0)code|=1u<<c;
