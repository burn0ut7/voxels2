...
// Active-range clear; the diagnostic proof also uses AllocationPass for its descriptor scan.
CS
{
	#include "system.fxc"
	...
	int CellCount < Attribute( "CellCount" ); >; int EdgeSlotCount < Attribute( "EdgeSlotCount" ); >;
	int DescriptorOffset < Attribute( "DescriptorOffset" ); >; int TotalsOffset < Attribute( "TotalsOffset" ); >;
	int AllocationPass < Attribute( "AllocationPass" ); >; int BatchSize < Attribute( "BatchSize" ); >;
	[numthreads( 64, 1, 1 )]
