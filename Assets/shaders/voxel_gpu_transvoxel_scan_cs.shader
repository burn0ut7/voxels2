...
CS
{
	...
	[numthreads(256,1,1)] void MainCs(uint3 group:SV_GroupID,uint lane:SV_GroupIndex)
	{
		...
		uint block=group.x/(uint)EdgeGroupCount,g=group.x-block*(uint)EdgeGroupCount;if(block>=(uint)BatchSize)return;uint local=g*256+lane;
		Scan[lane]=local<(uint)EdgeSlotCount?EdgeFlags[block*(uint)EdgeSlotCount+local]:0;GroupMemoryBarrierWithGroupSync();
		for(uint up=1;up<256;up<<=1){uint i=(lane+1)*up*2-1;if(i<256)Scan[i]+=Scan[i-up];GroupMemoryBarrierWithGroupSync();}
		uint total=Scan[255];if(lane==0)Scan[255]=0;GroupMemoryBarrierWithGroupSync();
		for(uint down=128;down>0;down>>=1){uint i=(lane+1)*down*2-1;if(i<256){uint value=Scan[i-down];Scan[i-down]=Scan[i];Scan[i]+=value;}GroupMemoryBarrierWithGroupSync();}
		if(local<(uint)EdgeSlotCount)EdgeVertexIds[block*(uint)EdgeSlotCount+local]=Scan[lane];if(lane==0)EdgeGroupSums[block*(uint)EdgeGroupCount+g]=total;
	}
}
